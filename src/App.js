...
function App(props) {
  ...
  const [filter, setFilter] = useState('All');
    ...
    function editTask(id, newName, newTime) {
        const editedTaskList = tasks.map(task => {
            // if this task has the same ID as the edited task
            if (id === task.id) {
                //
                return {...task, name: newName, time: newTime}
            }
            return task;
        });
    ...
    const filterList = FILTER_NAMES.map(name => (
        <FilterButton
            key={name}
            name={name}
            //TODO: ADD TIME HERE IFFFFFF we want to filter by time
            isPressed={name === filter}
            setFilter={setFilter}
        />
    ));
  function addReminder(name, time) {
    const newTask = { id: "todo-" + nanoid(), name: name, time: time, completed: false };
